...
import { ArrowLeft, HardDrive, Shield, CheckCircle2, ChevronLeft, ChevronRight } from 'lucide-react';
      ...
      <div className="bg-white dark:bg-[#121212] rounded-[2.5rem] p-6 md:p-12 lg:p-16 border border-gray-100 dark:border-gray-800 shadow-sm">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 items-center">
          ...
          <div className="w-full lg:w-1/2">
            <div className="relative w-full pt-[100%] bg-gray-50 dark:bg-[#1a1a1a] rounded-[2rem] overflow-hidden group">
              {images.length > 0 ? (
                <>
                  <img 
                    src={images[currentImageIndex]} 
                    alt={`${product.name} - Image ${currentImageIndex + 1}`}
                    className="absolute inset-0 w-full h-full object-contain p-10 mix-blend-multiply dark:mix-blend-normal transition-opacity duration-300"
                  />
                  ...
                  {images.length > 1 && (
                    <>
                      <button 
                        onClick={prevImage}
                        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 dark:bg-black/80 backdrop-blur text-black dark:text-white p-2.5 rounded-full shadow-lg hover:scale-110 active:scale-95 transition-all outline-none"
                      >
                        <ChevronLeft className="w-5 h-5" />
                      </button>
                      <button 
                        onClick={nextImage}
                        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 dark:bg-black/80 backdrop-blur text-black dark:text-white p-2.5 rounded-full shadow-lg hover:scale-110 active:scale-95 transition-all outline-none"
                      >
                      ...
                      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2">
                        {images.map((_, idx) => (
                          <button
                            ...
                            className={`transition-all duration-300 rounded-full ${currentImageIndex === idx ? 'w-6 h-1.5 bg-black dark:bg-white' : 'w-1.5 h-1.5 bg-gray-400 dark:bg-gray-600 hover:bg-gray-600 dark:hover:bg-gray-400'}`}
                          />
                        ))}
                      </div>
                    </>
              ...
              <div className="flex items-center gap-3 mt-4 overflow-x-auto pb-2 scrollbar-hide">
                {images.map((img, idx) => (
                  <button 
                    key={idx}
                    onClick={() => setCurrentImageIndex(idx)}
                    className={`relative w-24 h-24 shrink-0 rounded-2xl overflow-hidden border-2 transition-all ${currentImageIndex === idx ? 'border-blue-500 shadow-md transform scale-[1.02]' : 'border-transparent hover:border-gray-200 dark:hover:border-gray-700'}`}
                  >
          ...
          <div className="w-full lg:w-1/2 flex flex-col justify-center">
            ...
            <p className="text-3xl lg:text-4xl font-semibold text-gray-900 dark:text-gray-100 tracking-tight mb-10">
              {formattedPrice}
            </p>
